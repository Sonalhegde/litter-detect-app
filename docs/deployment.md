...
The frontend is a Vite build deployed to Vercel with `pnpm run build:frontend` and `dist/public` as its output directory. Set `VITE_INFERENCE_API_URL` to the Render backend origin. The backend is a Docker web service deployed from `backend/Dockerfile` through `render.yaml`; set `CORS_ALLOWED_ORIGINS` to the exact Vercel origin.
