...
The URL is defined as the workflow's `RENDER_HEALTH_URL` environment variable and must match the origin the production frontend calls (`client/src/lib/detection.ts` → `API_BASE_URL`). The previous URL (`sentinal-yhe0.onrender.com`) pointed at the retired Node backend with no scene-relevance checker; after consolidating to the Python/FastAPI backend, the keep-alive now pings the single Python service directly.
